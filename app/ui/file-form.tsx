...
export default function FileForm(){
    ...
    return (
        <form action={formAction} className="flex flex-col justify-evenly items-center h-9/10 w-full">
            ...
            <div onClick={openFileDialog} className="hover:cursor-pointer hover:bg-sky-100 hover:border-sky-400 transition duration-200 ease-in-out flex flex-col items-center justify-center h-7/10 w-9/10 border-dashed border-2 border-red-500 rounded-lg">
                ...
                <label onMouseOver={() => setClickable(false)} onMouseLeave={() => setClickable(true)} htmlFor="file" className="z-1000 hover:cursor-pointer hover:bg-red-600 transition duration-100 bg-red-500 rounded w-1/5 p-3 text-center text-white">
                    {
                        fileName != "No files chosen" ? "Video selected" : "Select video"
                    }
                </label>
                <p>{fileName}</p>
                <button onMouseOver={() => setClickable(false)} onMouseLeave={() => setClickable(true)} className="my-5 z-1000 hover:cursor-pointer hover:bg-red-600 transition duration-100 bg-red-500 rounded w-1/5 p-3 text-center text-white">Upload a video</button>
                {state && <div>{state}</div>}
            </div>
        </form>
    )
